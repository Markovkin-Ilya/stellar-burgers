import { orderSlice, initialState, setOrederIngredients, deleteOrderState } from "./slice"
...
describe('Проверка слайса orderSlice', () => {
    ...
    it('deleteOrderState', () => {
        const prevState = { ...initialState, orderModalData: orderModalData, orderRequest: true, orderIngredients: orderId }
    ...
    it('orderBurger fulfilled', () => {
        const action = { type: orderBurger.fulfilled.type, payload: orderModalData };
        const state = orderSlice.reducer(initialState, action);
        expect(state).toEqual({ ...initialState, orderRequest: false, orderModalData: orderModalData });
    })
})